...
export class AppDetailsComponent implements OnInit {
  ...
  createForm() {
    this.addNewCustomereForm = new FormGroup({
      ...
      firstName: new FormControl("", [
        Validators.required,
        Validators.pattern(Constants.patterns.onlyTextWithoutSpace)
      ]),
      lastName: new FormControl(""),
      countOfPurchase: new FormControl("", [
        Validators.required,
        Validators.pattern(Constants.patterns.numberValidator)
      ]),
      amountOfItems: new FormControl("", [
        Validators.required,
        Validators.pattern(Constants.patterns.numberValidator)
      ])
    });
    ...
    if (this.customerToEdit) {
      const filteredEditedData = this._appService.listOfCustomer
        .filter(
          cust =>
            cust.customerId ===
            this.addNewCustomereForm.controls["customerId"].value
        )
        .map(tranCust => {
          return (tranCust = this.addNewCustomereForm.value);
        });
      const index = this._appService.listOfCustomer.indexOf(
        this._appService.listOfCustomer.filter(
          cust =>
            cust.customerId ===
            this.addNewCustomereForm.controls["customerId"].value
        )[0]
  ...
  cancel() {
    this._appService.publishEvents("CANCEL_ADD_CUSTOMER_CLICK", {
      EVENT: "CANCEL_ADD_CUSTOMER_CLICK",
      CLICKED: true,
